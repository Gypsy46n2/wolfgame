  ...
  PausableUpgradeable
{
  ...
  - The game logic trades off gas efficiency for precision. Supply will converge to approximately 2.4b tokens when everybody has claimed. If the number of sheep choosing risk game is uneven and very small, the final amount will somewhat deviate from that number, but in practice the difference should be negligible.
  ...
  */
  ...
  uint128 constant TOTAL_SHEEP = 12067;
  uint8 constant MAX_ALPHA = 8;
  uint128 constant TOTAL_ALPHA = 9897; // GET EXACT NUMBER
  ...
  uint128 constant TOTAL_UNSTAKED_EARNINGS = UNSTAKED_EARNINGS * 311; // NUMBER OF SHEEP THAT WERE UNSTAKED
  uint128 constant TOTAL_STAKED_EARNINGS = 786726742013888888888883155; // PULLED DIRECTLY FROM THE BLOCKCHAIN
   ...
   */
  function playItSafe(uint256[] calldata tokenIds, bool separatePouches) external {
    uint128 earned;
    uint128 temp;
   ...
   * @param tokenIds the Sheep to opt in
   */
  function takeARisk(uint256[] calldata tokenIds) external {
   ...
   * @param separatePouches whether or not to give a single Pouch or one for each Sheep
   */
  function executeRisk(uint256[] calldata tokenIds, bool separatePouches) external {
   ...
   */
  function claimWolfEarnings(uint256[] calldata tokenIds, bool separatePouches) external {
    require(randomSeed != 0x0, "CANT EXECUTE YET");
    uint128 earned;
    uint128 alpha;
    uint128 temp;
